...
def print_results_tables(records, selection_method, latex):
    ...
    for dataset in dataset_names:
        ...
        alg_train_pairs = [(alg,train_env) for alg in alg_names for train_env in train_envs]

        table = [[None for _ in [*test_envs, "Avg"]] for _ in alg_train_pairs]
        for i, (algorithm,train_env) in enumerate(alg_train_pairs):
            means = []
            for j, test_env in enumerate(test_envs):
                trial_accs = (grouped_records
                    .filter_equals(
                        "dataset, algorithm, train_env, test_env",
                        (dataset, algorithm, train_env, test_env)
                    ).select("sweep_acc"))
        ...
        train_test_names = [env_names[i]+'_'+env_names[j] for i in train_envs for j in test_envs]
        col_labels = [
            "Algorithm", 
            *env_names,
            "Avg"
        ]
        row_labels = [alg+'_source'+env_names[train_env] for (alg,train_env) in alg_train_pairs]
        header_text = (f"Dataset: {dataset}, "
            f"model selection method: {selection_method.name}")
        print_table(table, header_text, row_labels, list(col_labels),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Domain adaptation testbed")
    parser.add_argument("--input_dir", type=str, nargs='+', default=[''])
    ...
    for dir in args.input_dir:
        records += reporting.load_records(dir)
