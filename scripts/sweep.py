...
class Job:
    ...
    @staticmethod
    def launch_multi_gpu(jobs, launcher_fn, gpus):
        print('Launching...')
        jobs = jobs.copy()
        np.random.shuffle(jobs)
        print('Making job directories:')
    ...
    for trial_seed in range(n_trials):
        for algorithm in algorithms:
            for dataset in dataset_names:
                all_envs = list(range(datasets.num_environments(dataset)))
                all_envs = [[i] for i in all_envs]
                if dataset in ['RotatedMNIST','SVHNMNIST','VisDA17']:
                    train_envs_pool = [[0]]
                else:
                    train_envs_pool = all_envs

                for train_envs in train_envs_pool:
                    for test_envs in all_envs:
                        if train_envs != test_envs:
                            for hparams_seed in range(n_hparams_from, n_hparams):
                                train_args = {}
                                train_args['dataset'] = dataset
                                train_args['algorithm'] = algorithm
                                train_args['test_envs'] = test_envs
                                train_args['train_envs'] = train_envs
...
if __name__ == "__main__":
    ...
    if args.command == 'launch':
        ...
        if args.command_launcher=='multi_gpu':
            Job.launch_multi_gpu(to_launch, launcher_fn, args.gpus)
        else:
